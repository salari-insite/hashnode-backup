...
To not overflow this article with information, I'm going to split this information into two articles. I'll cover the first 5 concepts in this article, and the last 4 concepts in next week's article.
...
Please keep in mind that I'll only be scratching the surface on all of these concepts. If you're self-teaching linear algebra to advance your
...
The dimension of a vector is important in considering whether a vector exists within a particular vector space. Before I go into depth about vector spaces, I'm going to cover some vector operations and definitions so you can understand what vector spaces are.
...
Vector spaces are important in linear transformations, vector scaling, and other vector operations that are important in linear algebra and computer science.
...
Vector-matrix multiplication is a foundational operation in data science. For instance, multiple linear regression models use vector-matrix multiplication to find a vector of regression coefficients to predict the outcome of a dependent variable in a dataset.
...
One of the most prevalent uses of transformations in computer science is in signal processing. In particular, there's a particular transformation called the Discrete Fourier Transformation which is used to transform a signal's time vector (or an image's digital signal vector of pixels for example) into a signal's frequency vector allowing for tasks such as image compression, audio editing, and in the transmission of digital signals.
...
In this article, I've given a crash course on the basic linear algebra concepts used in various branches of computer science.

Although, please keep in mind that I've only scratched the surface on all of these concepts. If you want more in-depth knowledge on these concepts and other concepts in linear algebra, I highly recommend [**Introduction to Linear Algebra by Gilbert Strang**](https://amzn.to/44VFQvs)**.** This is the same exact textbook that was used in my college linear algebra course. The structure of the content is highly organized and easily digestible for self-teaching.
...
As stated previously, I'll be covering the rest of the linear algebra concepts (orthogonality, determinants, eigenvalues and eigenvectors, and singular value deomposition) in a future article. To stay up to date and not miss out when I post new articles, make sure to subscribe to my newsletter!