...
Insertion sort is another slow sorting algorithm but is sometimes faster than bubble sort.
...
However, remember that the insertion sort's time complexity is only in the worst-case scenario where the list starts in reverse order, whereas the bubble sort's time complexity is for every case.
